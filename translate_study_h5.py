# -*- coding: utf-8 -*-
"""
Translate all study tour HTML files from bilingual (Chinese/English) to English-only.
Also fix prices for each region.
"""
import re, glob, os
...
# === PRICE MAPPING ===
# key: region identifier, value: [4d_price, 7d_price, 14d_price]
PRICES = {
    "港澳台": [7980, 13980, 21980],
    "东盟": [9980, 15980, 23980],      # keep original for ASEAN
    "俄罗斯": [9980, 15980, 23980],     # keep original for Russia
...
# === TRANSLATION MAP ===
# We'll do broad replacement patterns
# Each entry: (regex pattern, replacement)

# Entry page specific replacements
ENTRY_REPLACEMENTS = [
    # Title
    (r'<html lang="zh-CN">', '<html lang="en">'),
    # Cover eyebrow
    (r'古都南京 · 文脉探索营 / Nanjing Cultural Camp', 'Nanjing Cultural Camp'),
    # Cover subtitle
    (r'读万卷书，行万里路<br>在六朝古都中触摸中华文明的脉搏<br>\s*Read ten thousand books, walk ten thousand miles — Touch the pulse of Chinese civilization\.',
    ...
    # Cover meta labels
    (r'适用学段 / Target', 'Target Group'),
    (r'研学目的地 / Destination', 'Destination'),
    # Region-specific cover titles
    (r'>港澳台</div>\s*<span class="cover-title-accent">中华文化研学之旅 · 大学生专场</span>',
     '>Hong Kong, Macao & Taiwan</div>\n    <span class="cover-title-accent">Chinese Culture Study Tour for University Students</span>'),
    (r'>东盟</div>\s*<span class="cover-title-accent">.*?</span>',
     '>ASEAN</div>\n    <span class="cover-title-accent">Cultural Exchange Study Tour for University Students</span>'),
    (r'>俄罗斯</div>\s*<span class="cover-title-accent">.*?</span>',
     '>Russia</div>\n    <span class="cover-title-accent">Culture & Innovation Study Tour for University Students</span>'),
    (r'>欧洲大陆</div>\s*<span class="cover-title-accent">.*?</span>',
     '>Continental Europe</div>\n    <span class="cover-title-accent">Heritage & Innovation Study Tour for University Students</span>'),
    (r'>英国</div>\s*<span class="cover-title-accent">.*?</span>',
     '>United Kingdom</div>\n    <span class="cover-title-accent">British Culture & Education Study Tour for University Students</span>'),
    (r'>美国</div>\s*<span class="cover-title-accent">.*?</span>',
     '>United States</div>\n    <span class="cover-title-accent">American Innovation & Culture Study Tour for University Students</span>'),

    # Remove Chinese cover-title-en (since title is now English, no need for subtitle-en)
    # Actually keep cover-title-en but remove Chinese from it
]

def fmt_price(n):
    return f"{n:,}"

def fix_prices(content, region):
    """Fix prices in entry pages"""
    if region not in PRICES:
        return content
    p = PRICES[region]
    # Pattern: ¥X,XXX / ¥X,XXX etc
    # Replace prices in choose-price sections
    # We replace all 3 prices in order (4d, 7d, 14d)
    prices_found = re.findall(r'>¥[\d,]+<', content)
    if len(prices_found) == 3:
        content_new = content
        for i, pf in enumerate(prices_found):
            content_new = content_new.replace(pf, f'>¥{fmt_price(p[i])}<', 1)
        return content_new
    return content

def translate_entry_page(filepath, region):
    """Translate a single entry page to English"""
    ...
    content = re.sub(
        r'<title>.*?</title>',
        f'<title>TOPOO Study Tour · {get_en_region(region)} · Nanjing Cultural Camp</title>',
        content
    )

    # Cover eyebrow - remove Chinese
    content = re.sub(
        r'<div class="cover-eyebrow">.*?</div>',
        '<div class="cover-eyebrow">Nanjing Cultural Camp</div>',
        content
    )

    # Cover title - already region name in Chinese, replace
    if region == "港澳台":
        content = content.replace(
            '<div class="cover-title">港澳台</div>',
            '<div class="cover-title">Hong Kong, Macao & Taiwan</div>'
        )
    elif region == "东盟":
        content = content.replace(
            '<div class="cover-title">东盟</div>',
            '<div class="cover-title">ASEAN</div>'
        )
    elif region == "俄罗斯":
        content = content.replace(
            '<div class="cover-title">俄罗斯</div>',
            '<div class="cover-title">Russia</div>'
        )
    elif region == "欧洲大陆":
        content = content.replace(
            '<div class="cover-title">欧洲大陆</div>',
            '<div class="cover-title">Continental Europe</div>'
        )
    elif region == "英国":
        content = content.replace(
            '<div class="cover-title">英国</div>',
            '<div class="cover-title">United Kingdom</div>'
        )
    elif region == "美国":
        content = content.replace(
            '<div class="cover-title">美国</div>',
            '<div class="cover-title">United States</div>'
        )

    # Cover title accent - replace bilingual with English only
    content = re.sub(
        r'<span class="cover-title-accent">.*?</span>',
        f'<span class="cover-title-accent">{get_en_accent(region)}</span>',
        content
    )

    # Cover title en - remove (redundant now)
    content = re.sub(r'<span class="cover-title-en">.*?</span>\n?', '', content)
    ...
    # Cover meta
    content = content.replace('适用学段 / Target', 'Target Group')
    content = content.replace('研学目的地 / Destination', 'Destination')

    # Fix val "大学生" for overseas, "中小学生" for domestic
    if region != "国内中小学生":
        content = content.replace('<div class="val">大学生</div>', '<div class="val">University</div>')

    # Cover version
    content = re.sub(
        r'<div class="cover-version">.*?</div>',
        f'<div class="cover-version">TOPOO STUDY TOUR · {get_en_region(region)} · 2026</div>',
        content
    )

    # Nanjing section
    content = content.replace('City Profile / 城市介绍', 'City Profile')
    content = content.replace('南京城市介绍 / Nanjing City Profile', 'Nanjing City Profile')
    content = content.replace(
    ...
    # NJ cards
    content = content.replace('历史地位 / Historical Status', 'Historical Status')
    content = content.replace('科教实力 / Education', 'Education')
    content = content.replace('地理气候 / Climate', 'Climate')
    content = content.replace('产业名片 / Industry', 'Industry')

    # NJ card values - replace bilingual with English
    content = content.replace(
        '六朝古都 · 十朝都会',
    ...
    content = content.replace(
        ...
        'Approx. 2,500 years of city history, 450 years as a capital — one of China\'s Four Great Ancient Capitals alongside Xi\'an, Luoyang, and Beijing.'
    )
    content = content.replace(
        '中国高等教育第三城',
        '3rd Largest Higher Education Hub in China'
    )
    content = content.replace(
        '拥有53所高等院校，在校大学生超85万，仅次于北京、上海<br>53 universities, 850,000+ students — 3rd in China',
        '53 universities, 850,000+ students — ranked 3rd nationwide, after Beijing and Shanghai.'
    )
    content = content.replace(
        '北亚热带湿润气候',
    ...
    content = content.replace(
        ...
        'Four distinct seasons with pleasant spring and autumn. Average temperature 15.4\u00b0C — ideal for study tours.'
    )
    content = content.replace(
        '六朝古都 · 创新名城',
        'Heritage City & Innovation Hub'
    )
    content = content.replace(
        '拥有软件谷、江北新区等国家级园区，长三角特大城市<br>Home to Natl Software Valley, major Yangtze Delta city',
        'Home to the National Software Valley, Jiangbei New Area, and other national-level zones. A major city in the Yangtze River Delta.'
    )
    ...
    # Choose section
    content = content.replace('选择你的研学行程 / Choose Your Program', 'Choose Your Program')
    content = content.replace(
    ...
    # Choose cards - durations
    content = content.replace('4日3晚 / 4Days3Nights', '4 Days 3 Nights')
    content = content.replace('7日6晚 / 7Days6Nights', '7 Days 6 Nights')
    content = content.replace('14日13晚 / 14Days13Nights', '14 Days 13 Nights')

    # Choose card names
    content = re.sub(
        r'<div class="choose-name">文脉初探<span class="choose-name-accent">Cultural Primer / 初识金陵</span></div>',
        '<div class="choose-name">Cultural Primer</div>',
        content
    )
    content = re.sub(
        r'<div class="choose-name">深度探索<span class="choose-name-accent">Deep Dive / 深度体验</span></div>',
        '<div class="choose-name">Deep Dive</div>',
        content
    )
    content = re.sub(
        r'<div class="choose-name">全面沉浸<span class="choose-name-accent">Full Immersion / 全面体验</span></div>',
        '<div class="choose-name">Full Immersion</div>',
        content
    )

    # Price unit
    content = content.replace('/人 · per person', '/person')

    # Tags
    content = content.replace('大学生 / University', 'University')
    content = content.replace('中英双语 / Bilingual', 'Bilingual')

    # View details
    content = content.replace('查看详情 / View Details', 'View Details')
    ...
    # Projects section
    content = content.replace('Optional Programs / 备选项目', 'Optional Programs')
    content = content.replace(
        '定制特色服务 · 备选项目',
        'Optional Premium Programs'
    )
    content = content.replace(
    ...
    # Project items - translate all 16
    projects = [
    ...
    for zh_title, en_title, zh_desc, en_desc in projects:
        # Match pattern: <h5>ZH_TITLE</h5><p>ZH_DESC</p>
        pattern = rf'<h5>{re.escape(zh_title)}</h5>\s*<p>{re.escape(zh_desc)}</p>'
        replacement = f'<h5>{en_title}</h5><p>{en_desc}</p>'
        content = re.sub(pattern, replacement, content, flags=re.DOTALL)

    # Back cover
    content = content.replace('拓圈 / Topoo', 'Topoo')
    content = content.replace('让小而美被世界看见 / Let the Small and Beautiful Be Seen', 'Let the Small and Beautiful Be Seen')

    # Back cover info
    content = re.sub(
        r'古都文脉探索营 · [^·]+ · 入口页<br>\s*Nanjing Cultural Camp',
        f'Nanjing Cultural Camp',
        content
    )
    content = content.replace(
        '本方案最终解释权归主办方所有<br>Final interpretation right reserved by the organizer',
        'Final interpretation right reserved by the organizer'
    )

    # Fix prices
    content = fix_prices(content, region)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"  Translated entry: {os.path.basename(filepath)}")
...
def get_en_region(region):
    mapping = {
        ...
        "英国": "United Kingdom",
        "美国": "United States",
        "国内中小学生": "Domestic K-12",
    }
    return mapping.get(region, region)

def get_en_accent(region):
    mapping = {
        "港澳台": "Chinese Culture Study Tour for University Students",
    ...
    return mapping.get(region, "Study Tour for University Students")


def translate_sub_page(filepath, region):
    """Translate a sub page (4d/7d/14d) to English"""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    if "7日6晚" in filepath:
        duration = "7D6N"
    elif "14日13晚" in filepath:
        duration = "14D13N"
    content = re.sub(
        r'<title>.*?</title>',
        f'<title>TOPOO Study Tour · {get_en_region(region)} · {duration} · Nanjing Cultural Camp</title>',
        content
    )
    ...
    # Cover title (region name in Chinese)
    for cn, en in [("港澳台", "Hong Kong, Macao & Taiwan"), ("东盟", "ASEAN"), ("俄罗斯", "Russia"),
                    ("欧洲大陆", "Continental Europe"), ("英国", "United Kingdom"), ("美国", "United States")]:
        content = content.replace(f'<div class="cover-title">{cn}</div>', f'<div class="cover-title">{en}</div>')
    ...
    # Alternative subtitle pattern (some files may have slight variations)
    content = re.sub(
        r'读万卷书，行万里路<br>在六朝古都中触摸中华文明的脉搏<br>\s*Read ten thousand books.*?</div>',
        'Read ten thousand books, walk ten thousand miles.<br>Touch the pulse of Chinese civilization in the Ancient Capital of Six Dynasties.</div>',
    ...
    content = re.sub(
        r'<div class="cover-version">.*?</div>',
        f'<div class="cover-version">TOPOO STUDY TOUR · {get_en_region(region)} · {duration} · 2026</div>',
        content
    )
    ...
    # Nanjing description (bilingual -> English)
    content = content.replace(
        '南京，江苏省省会，地处长江下游中部，是中国四大古都之一，有"六朝古都、十朝都会"之称。全市面积6587平方公里，常住人口约950万。<br>\n      Nanjing, capital of Jiangsu Province, is one of China\'s Four Great Ancient Capitals, known as the "Ancient Capital of Six Dynasties and Ten Regimes". Total area: 6,587 km\u00b2, permanent population: approx. 9.5 million.',
        'Nanjing, capital of Jiangsu Province, is one of China\'s Four Great Ancient Capitals, known as the "Ancient Capital of Six Dynasties and Ten Regimes". Total area: 6,587 km\u00b2, permanent population: approx. 9.5 million.'
    )
    ...
    content = content.replace('地理气候 / Climate', 'Climate')
    content = content.replace('产业名片 / Industry', 'Industry')
    content = content.replace('六朝古都 · 十朝都会', 'Ancient Capital of Six Dynasties')
    content = content.replace(
        '近2500年建城史，450年建都史，与西安、洛阳、北京并称中国四大古都<br>Approx. 2,500 years of city history, 450 years as a capital',
        'Approx. 2,500 years of city history, 450 years as a capital — one of China\'s Four Great Ancient Capitals.'
    )
    content = content.replace('中国高等教育第三城', '3rd Largest Higher Education Hub in China')
    content = content.replace(
        '拥有53所高等院校，在校大学生超85万，仅次于北京、上海<br>53 universities, 850,000+ students — 3rd in China',
        '53 universities, 850,000+ students — ranked 3rd nationwide.'
    )
    content = content.replace('北亚热带湿润气候', 'Subtropical Humid Climate')
    content = content.replace(
        '四季分明，春秋宜人。年平均温度15.4°C，是理想研学目的地<br>Four distinct seasons, avg. temp 15.4\u00b0C — ideal for study tours',
        'Four distinct seasons with pleasant spring and autumn. Average temperature 15.4\u00b0C.'
    )
    content = content.replace('六朝古都 · 创新名城', 'Heritage City & Innovation Hub')
    content = content.replace(
        '拥有软件谷、江北新区等国家级园区，长三角特大城市<br>Home to Natl Software Valley, major Yangtze Delta city',
    ...
    # === Itinerary section ===
    content = content.replace('行程概览 / Itinerary Overview', 'Itinerary Overview')
    content = content.replace('行程概览 · 每日亮点', 'Itinerary Overview — Daily Highlights')

    # Day headers: "第X天 / Day X" -> "Day X"
    content = re.sub(r'第(\d+)天 / Day \1', r'Day \1', content)
    content = re.sub(r'第(\d+)天 · 主题', r'Day \1 · ', content)

    # Common day themes - translate
    day_themes = {
        '抵达 / Arrival': 'Arrival',
        '古都初见 / First Glimpse': 'First Glimpse of the Ancient Capital',
        ...
        '紫金山 / Purple Mountain': 'Purple Mountain',
        '历史街区 / Historic District': 'Historic District',
    ...
    for zh, en in day_themes.items():
        content = content.replace(zh, en)

    # Time slots
    content = content.replace('上午 / Morning', 'Morning')
    content = content.replace('下午 / Afternoon', 'Afternoon')
    content = content.replace('晚上 / Evening', 'Evening')
    content = content.replace('全天 / Full Day', 'Full Day')

    # Itinerary item titles (Chinese / English pattern -> English only)
    # Generic pattern: Chinese / English -> English
    # We'll do a broad pass: remove Chinese before " / " in certain contexts
    # This is tricky - let's handle specific common ones
    itinerary_items = [
        ...
        ('秦淮河夜游', 'Qinhuai River Night Cruise'),
    ...
    for zh, en in itinerary_items:
        # Replace both "ZH / EN" and standalone "ZH" patterns
        content = re.sub(rf'{re.escape(zh)}\s*/\s*[A-Za-z][A-Za-z\s,\.&\(\)]*', en, content)
        content = content.replace(zh, en)

    # Itinerary descriptions - bilingual patterns
    # Pattern: "Chinese desc<br>English desc" or "Chinese desc / English desc" -> keep English
    # This is the broadest pass
    content = re.sub(
        r'[\u4e00-\u9fff][\u4e00-\u9fff\s，。、；：（）·""！？\d°C]+\s*(?:<br>|/)\s*([A-Z][A-Za-z\s,.\'&\(\)\d°C]+)',
        r'\1',
        content
    )

    # === Include area (right sidebar of sub pages) ===
    include_items = [
        '费用包含 / What\'s Included',
        '住宿 / Accommodation',
        '餐饮 / Meals',
        '交通 / Transport',
        '门票 / Tickets',
        '课程 / Programs',
        '保险 / Insurance',
        '费用不含 / What\'s Not Included',
        '签证 / Visa',
        '个人消费 / Personal Expenses',
        '服务保障 / Our Services',
        '师生配比 / Teacher-Student Ratio',
        '安全措施 / Safety Measures',
        '应急机制 / Emergency Response',
        '安全保障 / Safety Assurance',
        '注意事项 / Important Notes',
        '退改政策 / Cancellation Policy',
        '报名须知 / Registration Notes',
        '行前准备 / Pre-departure Prep',
        '服务标准 / Service Standards',
    ]
    for item in include_items:
        parts = item.split(' / ')
        if len(parts) == 2:
            content = content.replace(item, parts[1])
            content = content.replace(f'{parts[1]} / {parts[0]}', parts[1])

    # More specific include text replacements
    include_text = [
    ...
    for zh, en in include_text:
        content = content.replace(zh, en)

    # Fix "住宿说明" type headers
    content = re.sub(r'住宿说明[^<]*', 'Accommodation Details', content)
    content = re.sub(r'餐饮说明[^<]*', 'Meal Arrangements', content)
    content = re.sub(r'交通说明[^<]*', 'Transportation', content)

    # Price display in sub pages
    content = content.replace('价格 / Price', 'Price')

    # Back cover
    content = content.replace('拓圈 / Topoo', 'Topoo')
    content = content.replace('让小而美被世界看见 / Let the Small and Beautiful Be Seen', 'Let the Small and Beautiful Be Seen')

    # Back cover credit/info
    content = re.sub(
        r'古都文脉探索营 · [^·<]+ · \d+日\d+晚',
        f'Nanjing Cultural Camp · {duration}',
        content
    )
    content = re.sub(
        r'Nanjing Cultural Camp · [^<·]+ · \d+D\d+N',
        f'Nanjing Cultural Camp · {duration}',
        content
    )
    content = content.replace(
    ...
    # Fix prices in sub pages too
    content = fix_prices(content, region)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"  Translated sub: {os.path.basename(filepath)}")
...
def main():
    ...
    for region in ["港澳台", "东盟", "俄罗斯", "欧洲大陆", "英国", "美国"]:
        for dur in ["4日3晚", "7日6晚", "14日13晚"]:
            ...
            sub_pages.append((fname, region))

    print("=== Translating entry pages ===")
    for fname, region in entry_pages:
        fpath = os.path.join(BASE, fname)
        if os.path.exists(fpath):
            translate_entry_page(fpath, region)
        else:
            print(f"  NOT FOUND: {fname}")

    print("\n=== Translating sub pages ===")
    for fname, region in sub_pages:
        fpath = os.path.join(BASE, fname)
        if os.path.exists(fpath):
            translate_sub_page(fpath, region)
        else:
            print(f"  NOT FOUND: {fname}")
